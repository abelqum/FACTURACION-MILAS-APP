...
import { X, Camera, UploadCloud, Plus, Save, Trash2, CheckCircle, AlertCircle, Clock, ExternalLink, Flag } from "lucide-react";
...
export default function ModalDetalleViaje({ isOpen, onClose, viaje, onActualizado, rolUsuario }) {
  ...
  const presupuestoViaticosTotal = Number(viaje.presupuesto_viaticos); 
  
  const presupuestoAlimentos = Number(viaje.presupuesto_comidas_diario) * 3 * Number(viaje.dias) * totalIntegrantes;
  const presupuestoRuta = presupuestoViaticosTotal - presupuestoAlimentos;
  
  const gastadoAlimentos = viaje.viaje_gastos
    ?.filter(g => g.categoria === 'Alimentos' && g.estatus === 'aprobado')
    .reduce((acc, g) => acc + Number(g.monto), 0) || 0;
    
  const gastadoRuta = viaje.viaje_gastos
    ?.filter(g => g.categoria !== 'Alimentos' && g.estatus === 'aprobado')
    .reduce((acc, g) => acc + Number(g.monto), 0) || 0;
  ...
  const handleGuardarTickets = async () => {
    const validos = nuevosGastos.filter(g => g.categoria && g.descripcion && g.monto && g.fotoArchivo);
    if (validos.length === 0) return Swal.fire("Atención", "Llena todos los campos (incluyendo foto) de al menos un ticket.", "warning");

    setCargando(true);
    Swal.fire({ title: "Subiendo tickets...", allowOutsideClick: false, didOpen: () => Swal.showLoading() });
    ...
    try {
      ...
      Swal.fire("Éxito", "Tickets enviados a revisión.", "success");
      setNuevosGastos([{ categoria: "", descripcion: "", monto: "", fotoArchivo: null, fotoPreview: null }]);
      onActualizado();
    } catch (error) {
      Swal.fire("Error", "Fallo al subir comprobantes.", "error");
    } finally {
      setCargando(false);
    }
  };
  ...
  const cambiarEstatusGasto = async (gastoId, nuevoEstatus) => {
    ...
    if (nuevoEstatus === 'rechazado') {
      const { value: razon } = await Swal.fire({
        ...
        input: 'textarea',
        inputPlaceholder: 'Ej. Ticket ilegible, gasto no autorizado...',
        showCancelButton: true,
        confirmButtonColor: "#ef4444",
  ...
  // 🟢 FUNCIÓN DE CIERRE DE CUENTAS
  const finalizarViaje = async () => {
    const pendientes = viaje.viaje_gastos?.filter(g => g.estatus === 'pendiente').length || 0;
    if (pendientes > 0) {
      return Swal.fire("Atención", "Aún tienes tickets pendientes de revisar. Apruébalos o recházalos antes de finalizar el viaje.", "warning");
    }

    const confirm = await Swal.fire({
      title: "¿Finalizar Viaje y Cerrar Cuentas?",
      text: "Se bloqueará la subida de tickets nuevos y se habilitará el reporte PDF definitivo.",
      icon: "warning",
      showCancelButton: true,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
      <div className="bg-slate-50 w-full max-w-6xl rounded-3xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden">
        ...
        <div className="p-6 bg-white border-b border-slate-200 flex justify-between items-center shrink-0">
          ...
          <button onClick={onClose} className="p-2 text-slate-400 hover:text-red-500 bg-slate-100 rounded-xl transition-all">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white border border-emerald-200 rounded-2xl p-5 shadow-sm">
              <h4 className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-3">Fondo Alimentos</h4>
              <div className="flex justify-between items-end">
                <div>
                  <p className={`text-2xl font-black ${presupuestoAlimentos - gastadoAlimentos < 0 ? 'text-red-600' : 'text-slate-800'}`}>
                    {formatearDinero(presupuestoAlimentos - gastadoAlimentos)}
                  </p>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Disponible</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-slate-600">{formatearDinero(gastadoAlimentos)} Aprobado</p>
                  <p className="text-xs text-slate-400 font-semibold">de {formatearDinero(presupuestoAlimentos)}</p>
                </div>
              </div>
              <div className="w-full bg-slate-100 h-2 mt-3 rounded-full overflow-hidden">
                <div className={`h-full ${presupuestoAlimentos - gastadoAlimentos < 0 ? 'bg-red-500' : 'bg-emerald-500'}`} style={{ width: `${Math.min((gastadoAlimentos / presupuestoAlimentos) * 100, 100)}%` }}></div>
              </div>
            </div>

            <div className="bg-white border border-blue-200 rounded-2xl p-5 shadow-sm">
              <h4 className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-3">Fondo Operación (Gas, Casetas, Mat.)</h4>
              <div className="flex justify-between items-end">
                <div>
                  <p className={`text-2xl font-black ${presupuestoRuta - gastadoRuta < 0 ? 'text-red-600' : 'text-slate-800'}`}>
                    {formatearDinero(presupuestoRuta - gastadoRuta)}
                  </p>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Disponible</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-slate-600">{formatearDinero(gastadoRuta)} Aprobado</p>
                  <p className="text-xs text-slate-400 font-semibold">de {formatearDinero(presupuestoRuta > 0 ? presupuestoRuta : 0)}</p>
                </div>
              </div>
              <div className="w-full bg-slate-100 h-2 mt-3 rounded-full overflow-hidden">
                <div className={`h-full ${presupuestoRuta - gastadoRuta < 0 ? 'bg-red-500' : 'bg-blue-500'}`} style={{ width: `${Math.min((gastadoRuta / presupuestoRuta) * 100, 100)}%` }}></div>
              </div>
            </div>
          </div>

          {/* 🟢 SOLO MOSTRAR FORMULARIO DE NUEVOS TICKETS SI EL VIAJE ESTÁ ACTIVO */}
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
              ...
              {nuevosGastos.map((gasto, idx) => (
                <div key={idx} className="flex flex-col md:flex-row gap-3 items-start md:items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                  ...
                  <div className="w-16 h-16 shrink-0 bg-white border border-slate-200 rounded-xl overflow-hidden relative flex items-center justify-center group">
                    {gasto.fotoPreview ? (
                  ...
                  <div className="w-full flex-1">
                    <input required type="text" placeholder="Ej.Comida, Caseta Tepotzotlan..." value={gasto.descripcion} onChange={(e) => actualizarGasto(idx, "descripcion", e.target.value)} className="w-full p-2 bg-white border border-slate-300 rounded-lg text-xs font-bold text-slate-700 focus:border-blue-600 outline-none" />
                  </div>
                  ...
                  <button type="button" onClick={() => quitarGastoFila(idx)} disabled={nuevosGastos.length === 1} className="p-2 shrink-0 text-slate-300 hover:text-red-500 bg-white border border-slate-200 rounded-lg disabled:opacity-30 self-end md:self-auto">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
              ...
              <div className="flex justify-end pt-2">
                <button onClick={handleGuardarTickets} disabled={cargando} className="px-6 py-2 bg-slate-800 text-white font-black rounded-xl text-xs uppercase tracking-widest shadow-md hover:bg-slate-900 active:scale-95 disabled:opacity-50 transition-all flex items-center gap-2">
                  {cargando ? "Subiendo..." : "Enviar Tickets a Revisión"} <Save size={14} />
                </button>
              </div>
            </div>
          )}
              ...
              viaje.viaje_gastos?.map(g => (
                <div key={g.id} className={`flex flex-col md:flex-row justify-between md:items-center p-4 rounded-xl border gap-4 transition-colors ${g.estatus === 'rechazado' ? 'bg-red-50/50 border-red-100' : 'bg-slate-50 border-slate-100'}`}>
                  ...
                  <div className="flex gap-4 items-center w-full md:w-auto">
                    <a href={g.foto_ticket_url} target="_blank" className="w-14 h-14 rounded-xl bg-white border border-slate-200 overflow-hidden relative group block shrink-0 shadow-sm">
                      <img src={g.foto_ticket_url} className="w-full h-full object-cover" />
                      <div className="absolute inset-0 bg-blue-900/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><ExternalLink size={18} className="text-white"/></div>
                    </a>
                  ...
                  <div className="flex flex-col items-end w-full md:w-auto gap-2">
                    <div className="flex items-center gap-4">
                      ...
                      {g.estatus === 'pendiente' && (
                        rolUsuario === 'admin' ? (
                          <div className="flex gap-2 shrink-0">
                            <button onClick={() => cambiarEstatusGasto(g.id, 'aprobado')} className="px-3 py-1.5 bg-emerald-100 text-emerald-700 hover:bg-emerald-600 hover:text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1">
                              <CheckCircle size={14} /> Aceptar
                            </button>
                            <button onClick={() => cambiarEstatusGasto(g.id, 'rechazado')} className="px-3 py-1.5 bg-red-100 text-red-700 hover:bg-red-600 hover:text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1">
                              <X size={14} /> Rechazar
                            </button>
                          </div>
                        ) : (
                      ...
                      {g.estatus === 'aprobado' && (
                        <span className="text-[10px] font-black px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-lg uppercase tracking-widest flex items-center gap-1 shrink-0"><CheckCircle size={14}/> Aprobado</span>
                      )}

                      {(rolUsuario === 'admin' || g.estatus !== 'aprobado') && viaje.estatus !== 'finalizado' && (
                        <button onClick={() => eliminarTicket(g)} className="p-2 text-slate-400 hover:text-red-500 bg-white border border-slate-200 rounded-lg shrink-0 transition-colors" title="Borrar Ticket">
                          <Trash2 size={16} />
                        </button>
                      )}
                    </div>
                    ...
                    {g.estatus === 'rechazado' && (
                      <div className="bg-white border border-red-200 p-2 rounded-lg text-right w-full md:max-w-xs mt-1">
                        <span className="text-[10px] font-black text-red-600 uppercase tracking-widest flex items-center justify-end gap-1 mb-0.5">
                          <AlertCircle size={12}/> Rechazado
                        </span>
                        <p className="text-[10px] text-slate-600 font-semibold leading-tight">
                          Motivo: {g.motivo_rechazo || "No especificado."}
                        </p>
                      </div>
                    )}
                  </div>
        ...
        {/* 🟢 BOTÓN DE FINALIZAR VIAJE SÓLO PARA ADMIN */}
        {rolUsuario === "admin" && viaje.estatus === "activo" && (
          <div className="p-6 bg-slate-50 border-t border-slate-200 shrink-0 flex justify-end">
            <button 
              ...
              disabled={cargando} 
              className="px-8 py-3 bg-emerald-600 text-white font-black rounded-xl text-xs uppercase tracking-widest shadow-lg shadow-emerald-600/30 hover:shadow-emerald-700/40 active:scale-95 disabled:opacity-50 transition-all flex items-center gap-2"
            >
